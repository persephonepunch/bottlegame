...
const obstacleMaterial = new THREE.MeshStandardMaterial({ color: '#333333', metalness: 0, roughness: 1 })
const wallMaterial = new THREE.MeshStandardMaterial({ color: '#f14682', metalness: 0, roughness: 0 })
...
{
    const bottle = useGLTF('/bottlenew.glb')

    bottle.scene.children.forEach((mesh) =>
    {
    ...
    return <group position={ position }>
        ...
        <RigidBody type="fixed" colliders="hull" position={ [ 0, 0, 0 ] } restitution={ 0.2 } friction={ 0 }>
            <primitive object={ bottle.scene } scale={ 10 } />
        </RigidBody>
    </group>
}
